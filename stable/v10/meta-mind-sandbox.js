/**
 * META-MIND SANDBOX CONTROLLER v3.1 (Hover Radial Menus)
...
class SandboxController {
    constructor(kernel, registry) {
        ...
        this.isMenuPinned = false; // If true, hover leave won't hide it
    ...
    initEvents() {
        ...
        vp.addEventListener('click', (e) => {
            // If clicking void, close menu
            if(!e.target.closest('.node') && !e.target.closest('.radial-btn')) {
                this.hideRadialMenu(true); // force hide
            }
        });

        // Menu Container Interaction (Stop hide timer when entering buttons)
    ...
    showRadialMenu(node, pinned = false) {
        ...
        // If already open for this node and we are just hovering, do nothing
        ...
        // Center the 200x200 menu on the 72x72 node
        // Node Center: screenPos.x + 36, screenPos.y + 36
        // Menu TopLeft: Center - 100
        ...
        // Render Buttons
        // Note: onclicks use global SC reference for simplicity in string templates
        menu.innerHTML = `
    ...
    startHideTimer() {
        ...
        this.menuHideTimer = setTimeout(() => {
            this.hideRadialMenu();
        }, 300); // 300ms grace period to move from node to button
    }
    ...
    handlePointerDown(e) {
        ...
        if (e.button === 0 || e.button === 1) {
            ...
            this.hideRadialMenu(true); // Pan forces hide
        }
    }
    ...
    render() {
        ...
        state.nodes.forEach(node => {
            ...
            // HOVER: Show Menu (Desktop)
            el.addEventListener('mouseenter', () => this.showRadialMenu(node, false)); // false = transient/hover mode
            el.addEventListener('mouseleave', () => {
                // If not pinned, schedule hide
                if (!this.isMenuPinned) this.startHideTimer();
            });

            // CLICK/TAP: Show Menu Pinned
            el.onclick = (e) => {
                e.stopPropagation();
                if(this.kernel.linkingMode) {
                    ...
                    this.kernel.selectNode(node.id);
                    this.showRadialMenu(node, true); // true = pinned
                }
            };
