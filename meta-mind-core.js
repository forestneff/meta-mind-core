/**
 * META-MIND CORE KERNEL v14.7
 * Features: Manifest-Driven Modular Library Loading.
 */
...
class HostBridge {
    ...
    // --- Phase 2: Native Library System ---
    async fetchTemplates() {
        try {
            // Uses the new global MetaMindLibrary instead of fetch()
            if (typeof MetaMindLibrary !== 'undefined') {
                return await MetaMindLibrary.getManifest();
            } else {
                throw new Error("MetaMindLibrary not loaded.");
            }
        } catch (e) {
            console.warn("Library failed to load. Ensure meta-mind-library.js is included.", e);
            return [];
        }
    }
    ...
    async fetchTemplateData(id) {
        try {
            if (typeof MetaMindLibrary !== 'undefined') {
                return await MetaMindLibrary.getTemplateData(id);
            } else {
                throw new Error("MetaMindLibrary not loaded.");
            }
        } catch (e) {
            console.error(`Failed to load template ${id}`, e);
            // Absolute minimal fallback so the app doesn't crash
            return {
                map_id: id, meta: { title: `Fallback: ${id}`, created: new Date().toISOString() },
                nodes: [{ id: `root_${id}`, type: "hub", title: "Error Loading Template", data: { x: 0, y: 0, isCore: true, collapsed: false } }],
                connections: [], submaps: []
            };
        }
    }
