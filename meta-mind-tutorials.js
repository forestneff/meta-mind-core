/**
 * META-MIND TUTORIAL ORCHESTRATOR
...
class TutorialOrchestrator {
    ...
    getTutorialProgress() {
        try {
            return JSON.parse(localStorage.getItem('metaMindTutorialProgress') || '{}');
        } catch(e) {
            return {};
        }
    }
    ...
    markTutorialComplete(id) {
        const prog = this.getTutorialProgress();
        prog[id] = true;
        localStorage.setItem('metaMindTutorialProgress', JSON.stringify(prog));
        this.updateLearnButtonState();
    }
